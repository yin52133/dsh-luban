...
for (const f of checklist.features ?? []) {
  ...
  if (!['P0', 'P1', 'P2', 'P3'].includes(f.priority)) problems.push(`checklist: ${f.id} bad priority "${f.priority}"`);
  if (!['todo', 'doing', 'review', 'done', 'blocked', 'dropped'].includes(f.status)) problems.push(`checklist: ${f.id} bad status "${f.status}"`);
}
const reqIds = new Set((checklist.requirements ?? []).map((r) => r.id));
const milestoneIds = new Set((checklist.milestones ?? []).map((m) => m.id));
for (const f of checklist.features ?? []) {
  if (!reqIds.has(f.requirement)) problems.push(`checklist: ${f.id} references unknown requirement "${f.requirement}"`);
  if (!milestoneIds.has(f.milestone)) problems.push(`checklist: ${f.id} references unknown milestone "${f.milestone}"`);
  if (f.id.slice(0, 3) !== f.module) problems.push(`checklist: ${f.id} module field "${f.module}" mismatches id prefix`);
}
...
for (const file of readdirSync(MODULES_DIR).filter((n) => n.endsWith('.md')).map((n) => join(MODULES_DIR, n))) {
  const text = readFileSync(file, 'utf8');
  for (const m of text.matchAll(tableRowRe)) docFeatureIds.add(m[1]);
}
const checkFeatureIds = new Set((checklist.features ?? []).map((f) => f.id));
for (const id of docFeatureIds) if (!checkFeatureIds.has(id)) problems.push(`ids: ${id} in module docs but missing from checklist.json`);
for (const id of checkFeatureIds) if (!docFeatureIds.has(id)) problems.push(`ids: ${id} in checklist.json but not found in any module spec table (design/03-modules)`);
...
for (const id of traceReqs) if (!checklistReqs.has(id)) problems.push(`trace-matrix: ${id} not defined in checklist.json requirements`);
for (const id of checklistReqs) if (!traceReqs.has(id)) problems.push(`trace-matrix: requirement ${id} not referenced in trace-matrix.md`);

// --- 5. milestone membership, both directions ---
for (const ms of checklist.milestones ?? []) {
  const listed = new Set(ms.featureIds ?? []);
  for (const fid of listed) if (!checkFeatureIds.has(fid)) problems.push(`milestone ${ms.id}: unknown feature ${fid}`);
  const expected = (checklist.features ?? []).filter((f) => f.milestone === ms.id).map((f) => f.id);
  for (const fid of expected) if (!listed.has(fid)) problems.push(`milestone ${ms.id}: feature ${fid} sets milestone="${fid}" field but missing from ${ms.id}.featureIds`);
  for (const fid of listed) {
    const f = (checklist.features ?? []).find((x) => x.id === fid);
    if (f && f.milestone !== ms.id) problems.push(`milestone ${ms.id}: ${fid} is also listed under milestone "${f.milestone}"`);
  }
}

// --- 6. designDoc paths exist ---
for (const f of checklist.features ?? []) {
  if (f.designDoc && !existsSync(join(ROOT, f.designDoc))) problems.push(`checklist: ${f.id} designDoc missing on disk: ${f.designDoc}`);
}
