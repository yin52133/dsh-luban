...
其他设备打不开时，先确认 Ubuntu 本机登录页正常，再检查监听地址和防火墙。自定义端口必须同时用于
防火墙规则和浏览器地址；不要开放内部上游 `3080`。完整的检查、放行和撤销步骤见
[防火墙与 Windows 访问排查](../../design/05-deployment/deploy-ubuntu.md#防火墙与-windows-访问排查)。
