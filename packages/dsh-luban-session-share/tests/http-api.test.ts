...
function headers(userId: string, role: 'admin' | 'operator' | 'observer'): HeadersInit {
  return { cookie: `user=${userId}; role=${role}`, 'content-type': 'application/json' }
}
  ...
  it('authenticates every route and enforces owner/operator/observer input rights', async (): Promise<void> => {
    ...
    try {
      ...
      const stream = await fetch(`${base}/sessions/S-http/events`, {
        headers: { cookie: 'user=bob; role=operator' },
      })
      ...
      const replay = await fetch(`${base}/sessions/S-http/events`, {
        headers: { cookie: 'user=bob; role=operator', 'last-event-id': '0' },
      })
