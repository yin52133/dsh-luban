...
import type { ChannelAdapter, ChannelEndpoint } from '@yin52133/dsh-luban-core'
...
describe('serial monitor, filter, capture and session injection', (): void => {
  it('redacts a selected bounded range and injects its file plus excerpt', async (): Promise<void> => {
    ...
    const service = new DefaultWinDebugService(testConfig(root), {
      ...
      accountSessions: memoryAccountSessions([[TEST_ACCOUNT, asSessionId('session-debug')]]),
    })
    ...
    expect(service.lines(TEST_ACCOUNT, channel.id, { query: 'FATAL' })).toHaveLength(1)
    expect(service.lines(TEST_ACCOUNT, channel.id, { query: '^boot', regex: true })).toHaveLength(1)
  ...
  it('bounds regex complexity and buffered line count', async (): Promise<void> => {
    ...
    expect(service.lines(TEST_ACCOUNT, channel.id).map((line): string => line.text)).toEqual([
      'two',
      'three',
    ])
    expect(() => service.lines(TEST_ACCOUNT, channel.id, { query: '(a+)+', regex: true })).toThrow(
      'too complex',
    )
  })
  ...
  it('fails closed on open/flash occupancy and probes a released COM port before execution', async (): Promise<void> => {
    ...
    await expect(service.open(TEST_ACCOUNT, 'serial:COM3')).rejects.toThrow(
    ...
    ).rejects.toThrow('serial port COM3 is occupied by another Luban channel')
  ...
  it('holds the device lease while channel open is in flight', async (): Promise<void> => {
    ...
    const opening = service.open(TEST_ACCOUNT, 'serial:COM3')
    await flush()
  ...
  it('keeps the device occupied until channel close and event pumping finish', async (): Promise<void> => {
    ...
    const closing = service.close(TEST_ACCOUNT, channel.id)
    const duplicateClosing = service.close(TEST_ACCOUNT, channel.id)
    await flush()
